...
sub responseFromFile
{
    ...
    ########################################################################################
    # Code fixups Start.  These are Code changes required to make Chilipeppr execute offline.

    # Override if statement logic to make loader happy
    $response =~ s/window.location.origin.indexOf\("fiddle.jshell.net"\) != -1/1/i if ($pathName =~ m@www.chilipeppr.com.js.app.js@);

    # Make the jsonp call into a simple json call, because the jsonp is random keyed and would never be correct in a static file.
    $response =~ s/&callback=\?//im if ($getURL =~ /2H9us/);

    if ($getURL =~ /dataget/)
    {
        # Add an Allow Origin header to make getJSON happy
...
sub fixupFileName
{
    # Special fixup for filenames of chilipeppr.com/geturl?url=blah requests.
    ...
    # Special fixup for filenames of chilipeppr.com/dataget?key=userUrl:blah requests.
    # GET http://www.chilipeppr.com/dataget?key=userUrl:tinyg HTTP/1.1
